...
// 预计算并缓存权重
fn precompute_weights(in_size: u32, out_size: u32) -> Vec<(i32, [f32; 4])> {
    let scale = in_size as f32 / out_size as f32;
    (0..out_size)
        .map(|i| {
            let f = (i as f32 + 0.5) * scale - 0.5;
            let base = f.floor() as i32 - 1;
            let frac = f - f.floor();
            let weights = [
                cubic_kernel(1.0 + frac),
                cubic_kernel(frac),
                cubic_kernel(1.0 - frac),
                cubic_kernel(2.0 - frac),
            ];
            (base, weights)
        })
        .collect()
...
//         let mean = Simd::from_array([self.mean[0], self.mean[1], self.mean[2], 0.0]);
//         let std = Simd::from_array([self.std[0], self.std[1], self.std[2], 1.0]);

//         for (x, y, pixel) in resized.enumerate_pixels() {
//             let val = Simd::from_array([
//                 pixel[0] as f32 / 255.0,
//                 pixel[1] as f32 / 255.0,
//                 pixel[2] as f32 / 255.0,
//                 0.0,
//             ]);
//             let normed = (val - mean) / std;
//             for c in 0..3 {
//                 arr[[c, y as usize, x as usize]] = normed[c];
//             }
//         }
...
// pub fn resize_bicubic(input: &RgbImage, out_w: u32, out_h: u32) -> RgbImage {
//     let (in_w, in_h) = input.dimensions();
//     let mut out = RgbImage::new(out_w, out_h);
...
//                     let pixel = input.get_pixel(sx as u32, sy as u32);
//                     let px = Simd::from_array([
//                         pixel[0] as f32,
//                         pixel[1] as f32,
//                         pixel[2] as f32,
//                         0.0, // padding
//                     ]);
//                     let weight = Simd::splat(wx * wy);
//                     acc += px * weight;
//                 }
//             }
