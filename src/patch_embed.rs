// 这个模块负责将 (3, H, W) 的 tensor 切分为 patch，然后映射到 embedding 维度。
...
//     pub embed_dim: usize,
//     pub weight: Array2<f32>, // shape: [embed_dim, patch_size * patch_size * 3]
...
//                 ]);
//                 let flat: Vec<f32> = patch.iter().copied().collect();
//                 patches.push(flat);
//             }
//         }

//         let input = ndarray::Array2::from_shape_vec((ph * pw, self.patch_size * self.patch_size * 3), patches.concat()).unwrap();
...
//         let patches: Vec<Vec<f32>> = (0..ph * pw).into_par_iter()
//             .map(|idx| {
//                 let i = idx / pw;
//                 let j = idx % pw;
//                 let patch = img.slice(s![
...
//                     j * self.patch_size..(j + 1) * self.patch_size
//                 ]);
//                 patch.iter().copied().collect::<Vec<f32>>()
//             })
//             .collect();

//         let input = Array2::from_shape_vec((ph * pw, patch_dim), patches.concat()).unwrap();
//         let mut output = input.dot(&self.weight.t());
//         if let Some(bias) = &self.bias {
//             output += &bias.t();
//         }
